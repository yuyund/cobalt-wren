...
from langgraph_automation.workflows.adapter import execute_workflow
        ...
        *,
        ...
        try:
            prepared = self._preparer.prepare(
                workflow_kind=workflow_kind, dependencies=self._dependencies, config=config
            )
    ...
    *,
    ...
    try:
        ...
        normalized_config = _augment_enabled_plugins(
            ...
            discovered_plugin_names=tuple(plugin.metadata.name for plugin in effective_discovered),
        )
...
def _augment_enabled_plugins(
    ...
    *,
    ...
    discovered_plugin_names: tuple[str, ...],
) -> NormalizedPackageConfig:
    enabled = _merge_unique_plugins(
        config.plugins.enabled,
        (runtime_plugin_name, *explicit_plugin_names, *discovered_plugin_names),
    )
