...
| `runtime` | Assemble dependencies from validated config; no workflow execution | `src/langgraph_automation/runtime/assembly.py`, `dependencies.py`, `secrets.py`, `context.py` | `tests/unit/runtime/*`, `tests/unit/architecture/test_runtime_assembly_boundary.py` | `CODE_CONFIRMED` + `TEST_CONFIRMED` + `ARCH_GUARD_CONFIRMED` | `GAP`: runtime assembly exists, but control-plane service modules still depend on `graphs.*` directly |
...
| `apps/automation` | Control-plane should ultimately route through `api.engine`, but some services still use graph/runtime internals directly | `src/langgraph_automation/apps/automation/services/runtime.py`, `execution.py`, `runs.py`, `workflow_config.py`, `workflow_preparation.py` | `tests/unit/apps/automation/services/*`, `tests/unit/architecture/test_apps_automation_package_boundary.py`, `test_service_workflow_integration_boundary.py` | `CODE_CONFIRMED` + `TEST_CONFIRMED` | `P0`: direct `graphs.*` and `workflows.catalog` imports still exist in service modules outside the preparation bridge |
...
| `Workflow.definition_payload` is parsed into graph-local runtime config and then into `GraphRuntime` | `apps/automation/services/runtime.py`, `apps/automation/services/workflow_config.py`, `graphs/config.py`, `graphs/runtime.py` | `tests/unit/automation/test_runtime_factory.py` | `CODE_CONFIRMED` + `TEST_CONFIRMED` | `P0`: this path still bypasses `api.engine` and imports `graphs.*` directly |
...
| --- | --- | --- | --- | --- |
| Config error | `config/loader.py`, `normalizer.py`, `security.py` | `ConfigError` with safe message and metadata | `CODE_CONFIRMED` + `TEST_CONFIRMED` | `CONTRACT_DRIFT`: public docs still mention a workflow error type that is not exported |
...
| Runtime assembly error | `runtime/assembly.py`, `runtime/secrets.py`, `api/engine.py` | `RuntimeAssemblyError` | `CODE_CONFIRMED` + `TEST_CONFIRMED` | `ASSUMED`: error taxonomy is stable, but the control plane still assembles runtime directly outside `api.engine` |
| Workflow preparation error | `workflows/prepare.py`, `workflows/requirements.py`, `workflows/adapter.py`, `api/engine.py` | `PluginResolutionError` or `RuntimeAssemblyError` | `CODE_CONFIRMED` + `TEST_CONFIRMED` | `GAP`: direct control-plane execution path still bypasses the facade |
...
| Secondary observability failures do not overwrite the primary failure | `integrations/observability/failure_policy.py`, `graphs/runner.py`, `integrations/llm/observed_client.py`, `integrations/tools/observed_registry.py`, `apps/automation/services/runs.py` | `tests/unit/automation/test_run_failure_observability_masking.py`, `tests/unit/integrations/test_observed_llm_client.py`, `tests/unit/graphs/test_runner.py` | `CODE_CONFIRMED` + `TEST_CONFIRMED` | `GAP`: control-plane services still assemble runtime directly in non-facade paths |
