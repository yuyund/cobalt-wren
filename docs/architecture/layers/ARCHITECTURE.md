...
- workflow config
- runtime assembly
- safe summary exposure for admin and dynamic UI
...
- UI layer
- renders explicit safe summaries only
...
- plugin taxonomy and responsibility boundaries are defined in `../../plugins/PLUGINS.md`.
- manual registration, enabled plugins, and registry conflict policy are defined in `../../plugins/PLUGIN_REGISTRATION.md`.
- validation hooks and factory hooks are defined in `../../plugins/PLUGIN_API_SHAPE.md`.
- plugin API facade staging is defined in `../../plugins/PLUGIN_API_FACADE.md`.
- workflow API facade staging is defined in `../../api/surface/API_SURFACE.md`.
- error categories and primary failure preservation are defined in `../../contracts/errors/ERROR_TAXONOMY.md`.
...
- error taxonomy and `api.errors` staging are defined in `../../contracts/errors/ERROR_TAXONOMY.md` and `../../api/errors/API_ERRORS_FACADE.md`.
