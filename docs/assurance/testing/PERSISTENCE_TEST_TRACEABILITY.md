...
| CheckpointStore protocol sufficiency is blocked | versioned checkpoint contract | `src/langgraph_automation/integrations/checkpoint/base.py`, `src/langgraph_automation/integrations/checkpoint/memory_store.py` | `tests/unit/architecture/test_checkpoint_store_protocol_sufficiency.py`, `tests/unit/docs/test_checkpoint_store_protocol_sufficiency_docs.py` | L1 protocol sufficiency audit | `BLOCKED_BY_PROTOCOL` | missing versioned identity, history, lineage, serializer compatibility, and specific-version reads | high |
...
- current tests now prove the checkpoint protocol is blocked by missing versioned durability capabilities
