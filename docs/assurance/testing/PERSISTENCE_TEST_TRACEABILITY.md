...
| Deployment startup equality is based on normalized binding signatures | application bootstrap | `src/langgraph_automation/apps/automation/apps.py`, `src/langgraph_automation/apps/automation/services/runtime.py` | `tests/unit/automation/test_persistence_deployment_startup.py` | L1 / L5 | `TEST_CONFIRMED` | equivalent default vs explicit `memory` store config remains equal; divergent filesystem roots still fail closed | medium |
| Concrete store construction is runtime-assembly scoped | runtime assembly timing | `src/langgraph_automation/runtime/artifact_store.py`, `src/langgraph_automation/runtime/checkpoint_store.py`, `src/langgraph_automation/apps/automation/services/runtime.py` | `tests/unit/runtime/test_persistence_construction_timing.py` | L1 / L3 runtime timing | `TEST_CONFIRMED` | startup binds config and runtime factory only; backend constructors run once per runtime build | medium |
