...
The plugin-specific validation rules for workflow, tool, provider, store, worker, and UI config are defined in `../../plugins/PLUGINS.md`.
Plugin-specific validation hooks are defined conceptually in `../../plugins/PLUGIN_API_SHAPE.md`.
Config validation invokes those hooks after raw schema, security, and semantic validation.
Configuration and plugin-specific validation failures are categorized in `../../contracts/errors/ERROR_TAXONOMY.md`.
