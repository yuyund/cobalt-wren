...
from cobalt_wren.api.stores import ArtifactReadResult
...
from cobalt_wren.api.plugins import (
    ...
    PluginContributions,
    PluginMetadata,
    discover_plugins as load_discovered_plugins,
...
from cobalt_wren.workflows.prepare import WorkflowPreparer

__all__ = ["EnginePreparedWorkflow", "AutomationEngine", "create_engine"]

_ENGINE_COMPONENT = "api_engine"
_INTERNAL_RUNTIME_PLUGIN_NAME = "cobalt_wren.engine_runtime_defaults"
...
class EnginePreparedWorkflow:
    """Public workflow handle retaining an opaque implementation object."""
    ...
    def execute(
        ...
        *,
        ...
        return execute_workflow(self.executable, input_payload, context=context)
    ...
    def resume(
        ...
        *,
        ...
        return resume_workflow(self.executable, request, context=context)
...
class AutomationEngine:
    ...
    def __init__(self, *, validated_config: ValidatedPackageConfig, dependencies: RuntimeDependencies, registry: PluginRegistry) -> None:
        self._validated_config = validated_config
        self._dependencies = dependencies
        self._registry = registry
        self._preparer = WorkflowPreparer(registry)

    def prepare_workflow(self, workflow_kind: str, *, config: Mapping[str, object] | None = None) -> EnginePreparedWorkflow:
        try:
            prepared = self._preparer.prepare(workflow_kind=workflow_kind, dependencies=self._dependencies, config=config)
        except FrameworkError:
            raise
        except Exception as exc:
            raise RuntimeAssemblyError("Engine workflow preparation failed.", code="ENGINE_WORKFLOW_PREPARATION_FAILED", component=_ENGINE_COMPONENT, metadata={"workflow_kind": workflow_kind}) from exc
...
def create_engine(config: Mapping[str, object], *, plugins: Sequence[Plugin] = (), discover_plugins: bool = False) -> AutomationEngine:
    try:
        ...
        discovered = load_discovered_plugins() if discover_plugins else ()
        registered_names = {plugin.metadata.name for plugin in registry.list_plugins()}
        effective_discovered: list[Plugin] = []
        for plugin in discovered:
            if plugin.metadata.name in registered_names:
                continue
            registry.register(plugin)
            registered_names.add(plugin.metadata.name)
            effective_discovered.append(plugin)
        normalized_config = normalize_package_config(load_package_config_from_mapping(config))
        normalized_config = _augment_enabled_plugins(normalized_config, runtime_plugin_name=runtime_defaults_plugin.metadata.name, explicit_plugin_names=tuple(plugin.metadata.name for plugin in plugins), discovered_plugin_names=tuple(plugin.metadata.name for plugin in effective_discovered))
        ...
        raise RuntimeAssemblyError("Engine creation failed.", code="ENGINE_CREATE_FAILED", component=_ENGINE_COMPONENT) from exc


def _augment_enabled_plugins(config: NormalizedPackageConfig, *, runtime_plugin_name: str, explicit_plugin_names: tuple[str, ...], discovered_plugin_names: tuple[str, ...]) -> NormalizedPackageConfig:
    enabled = _merge_unique_plugins(config.plugins.enabled, (runtime_plugin_name, *explicit_plugin_names, *discovered_plugin_names))
...
def _resolve_model_name(config: ProviderProfileConfig) -> str:
    ...
    if not model:
        raise RuntimeAssemblyError("Runtime assembly failed: provider model is required.", code="RUNTIME_ASSEMBLY_INVALID_CONFIG", component=_ENGINE_COMPONENT, metadata={"provider": config.provider})
    return model
...
def _resolve_optional_secret(config: ProviderProfileConfig, context: object, secret_name: str) -> str | None:
    ...
    if resolver is None:
        raise RuntimeAssemblyError("Runtime assembly failed: secret resolver is unavailable.", code="RUNTIME_ASSEMBLY_SECRET_RESOLVER_MISSING", component=_ENGINE_COMPONENT, metadata={"provider": config.provider, "secret_name": secret_name})
