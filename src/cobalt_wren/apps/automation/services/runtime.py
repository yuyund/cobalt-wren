...
from cobalt_wren.api.plugins import DEFAULT_PLUGIN_ENTRY_POINT_GROUP, Plugin
...
from cobalt_wren.config.models import NormalizedPackageConfig
...
def _installed_plugin_entry_point_signature() -> tuple[tuple[str, str, str, str], ...]:
    entries = importlib_metadata.entry_points().select(
        group=DEFAULT_PLUGIN_ENTRY_POINT_GROUP
    )
...
def load_deployment_package_config_from_settings() -> Mapping[str, object]:
    ...
    config_file = str(getattr(django_settings, "COBALT_WREN_CONFIG_FILE", "") or "").strip()
    if config_file:
        ...
        config = getattr(django_settings, "COBALT_WREN", None)
    if config is None:
        return {"version": 1}
    if isinstance(config, str):
        try:
            ...
            raise ConfigError(
                "Configuration is invalid: COBALT_WREN must be valid JSON.",
                code="CONFIG_INVALID_MAPPING",
                component="automation_runtime",
            ) from exc
    if not isinstance(config, Mapping):
        raise ConfigError(
            "Configuration is invalid: COBALT_WREN must be a mapping.",
            code="CONFIG_INVALID_MAPPING",
            component="automation_runtime",
        )
    return config
