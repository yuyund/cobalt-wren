...
from cobalt_wren.apps.automation.ui.specs import ActionSpec
from cobalt_wren.apps.automation.models.artifact import Artifact
from cobalt_wren.apps.automation.models.run import Run
from cobalt_wren.apps.automation.ui.workflow_projection import build_resume_action_specs, dispatch_resume_action
from cobalt_wren.apps.automation.ui.integration_actions import build_integration_action_specs, dispatch_integration_action
...
def build_action_specs(model_key: str, obj: object | None, actor: object | None = None) -> list[ActionSpec]:
    ...
    if model_key == "runs" and isinstance(obj, Run):
        ...
        specs.extend(build_integration_action_specs(obj, actor=actor))
...
def dispatch_ui_action(model_key: str, object_id: int, action_name: str, actor: object | None, request: HttpRequest | None = None) -> object:
    ...
    if model_key == "runs" and action_name.startswith("integration-"):
        if request is None or not isinstance(obj, Run):
            raise LookupError("Integration action request is unavailable")
        return dispatch_integration_action(obj, action_name, request, actor=actor)
