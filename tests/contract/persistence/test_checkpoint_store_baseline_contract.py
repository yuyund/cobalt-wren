"""Reusable baseline contract tests for CheckpointStore implementations."""
...
def test_checkpoint_store_baseline_contract_suite(backend_spec) -> None:
    ...
    assert_checkpoint_round_trip(store)
    assert_checkpoint_missing_behavior(store)
    assert_checkpoint_run_isolation(store)
    assert_checkpoint_defensive_copy(store)
    assert_checkpoint_delete_contract(store)
    assert_checkpoint_latest_state_compatibility(store)
