...
def test_separate_distribution_installs_and_executes_via_entry_point(tmp_path: Path) -> None:
    ...
    for package in (repo, external):
        subprocess.run([str(python), "-m", "pip", "install", "--no-build-isolation", "--no-deps", str(package)], check=True, capture_output=True, text=True)
    script = '''
import json
...
'''
    ...
    assert json.loads(completed.stdout.strip()) == {"foundation_installed": True, "external_installed": True, "entry_points": ["acme"], "output": {"message": "wheel:REQ-9"}}
