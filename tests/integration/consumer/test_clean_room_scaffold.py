...
def test_scaffolded_consumer_builds_and_loads_from_wheels(tmp_path: Path) -> None:
    ...
    subprocess.run([sys.executable, "-m", "langgraph_automation", "init-workflow", "--name", "consumer-workflow", "--kind", "consumer.echo", "--output", str(generated)], cwd=repo, check=True, env={**os.environ, "PYTHONPATH": str(repo / "src")})
...
entry = next(x for x in metadata.entry_points().select(group='langgraph_automation.plugins') if x.name == 'consumer-workflow')
plugin = entry.load()()
