...
from langgraph_automation.apps.automation.models.event import RunEventLevel
from langgraph_automation.apps.automation.models.execution import ExecutionSpanStatus, ExecutionSpanType
...
def test_event_sink_creates_events_spans_and_metadata() -> None:
    ...
    run = Run.objects.create(workflow=workflow, name='run-sink', status=RunStatus.PENDING)
    sink = DjangoEventSink()

    sink.run_started(run.pk, message='run started', payload={'trace_id': 'abc'})
    graph_ref = sink.span_started(run.pk, ExecutionSpanType.GRAPH, 'demo-graph', node_name='graph', metadata={'phase': 'demo'})
    ...
    assert run.events.filter(event_type='run.started', level=RunEventLevel.INFO).exists()
    assert run.events.filter(event_type='semantic.planner.decision_made', level=RunEventLevel.INFO).exists()
    assert run.events.filter(event_type='semantic.planner.decision_made', span=node_span).exists()
...
def test_event_sink_bounds_and_redacts_long_payloads_and_summaries() -> None:
    ...
    run = Run.objects.create(workflow=workflow, name='run-sink-safe', status=RunStatus.PENDING)
    sink = DjangoEventSink()

    long_secret = 'Authorization: Bearer ' + ('x' * 500)
    graph_ref = sink.span_started(run.pk, ExecutionSpanType.GRAPH, 'demo-graph-safe', node_name='graph', metadata={'secret_token': long_secret})
    node_ref = sink.node_started(run.pk, 'planner', parent=graph_ref)
    sink.span_completed(graph_ref, output_summary=long_secret, metrics={'latency_ms': 123, 'nested': {'password': long_secret}}, metadata={'raw_path': '/tmp/secret.txt'})
    sink.artifact_created(run.pk, 'run-123/output.md', 'report', 'text', span=node_ref, metadata={'trace_id': long_secret})
