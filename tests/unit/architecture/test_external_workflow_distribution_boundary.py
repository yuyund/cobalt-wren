...
ALLOWED = ("langgraph_automation.api.plugins", "langgraph_automation.api.workflow")
...
def test_external_distribution_has_independent_project_metadata_and_entry_point() -> None:
    ...
    assert '[project.entry-points."langgraph_automation.plugins"]' in text
...
def test_external_distribution_imports_only_public_spi() -> None:
    ...
    for path in (ROOT / "src").rglob("*.py"):
        for module in collect_import_targets(path):
            if module.startswith("langgraph_automation") and not module.startswith(ALLOWED):
